...
class ChatNotification{
    ...
    func setChat(userInfo: [AnyHashable: Any]) -> LastMsgDtls {
        ...
        let chatobj = LastMsgDtls(LastMsgSender: sendername, Lastmsgtext: msgtext, LastMsgTime: senttime, LastmsgSenderimage: "", LastMessageSenderID: from, ThraedID: from, ThreadName: sendername)
        
        if(chatinitmethods.getThread(threadId: from)){
            
            chatinitmethods.updateThread(userID: from, chatObj: msgObj)
        }
        else{
